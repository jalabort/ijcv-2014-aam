...
from .transform import OrthoPDM, LinearWarp, OrthoMDTransform
...
class AAMFitter(object):
    ...
    def _fit(self, images, initial_shape, gt_shapes=None, max_iters=50,
        ...
        Returns
        -------
        algorithm_results: :class:`menpo.fit.fittingresult.FittingResult` list
...
class LucasKanadeAAMFitter(AAMFitter):
    ...
    def __init__(self, aam, algorithm=AIC_GN,
                 n_shape=None, n_appearance=None, **kwargs):

        super(LucasKanadeAAMFitter, self).__init__(
        ...
        for j, (am, sm) in enumerate(zip(self.aam.appearance_models,
            ...
            self._algorithms.append(
                algorithm(StandardAAMInterface, am, md_transform, **kwargs))


class LucasKanadeLinearAAMFitter(AAMFitter):

    def __init__(self, linear_aam, algorithm=AIC_GN,
                 n_shape=None, n_appearance=None, **kwargs):

        super(LucasKanadeLinearAAMFitter, self).__init__(
...
class LucasKanadePartsAAMFitter(AAMFitter):

    def __init__(self, parts_aam, algorithm=AIC_GN,
                 n_shape=None, n_appearance=None, **kwargs):

        super(LucasKanadePartsAAMFitter, self).__init__(
